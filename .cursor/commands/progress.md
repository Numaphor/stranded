...
Use Bash to check (avoids PowerShell Get-ChildItem issues on Windows). Glob respects .gitignore but .planning/ is often gitignored:

```bash
test -d .planning && echo "exists" || echo "missing"
```

If you must use PowerShell, use only: `Test-Path .planning` (do not use Get-ChildItem for this check).
