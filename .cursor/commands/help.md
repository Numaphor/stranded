...
**One command for the full flow:** `/gsd:flow` — Asks if your codebase is new, then runs the autopilot setup (map → lazy-mode → new-project → plan-milestone-all → autopilot). Stay for steps 2–3 (~15–30 min); step 4 launches execution so you can walk away. For existing projects, resumes and runs the next step.
...
**Full flow (one command):** `/gsd:flow` — Asks "Is your codebase new?" then runs the autopilot setup: new path (map → lazy-mode → new-project → plan-milestone-all → autopilot) or existing path (resume → next step → keep going).
...
**`/gsd:flow`**
One command for the entire GSD workflow (autopilot setup for new codebases).

- Asks once: "Is this a new codebase?"
- **New (autopilot):** Map (if existing code) → **lazy-mode** → **new-project** (stay — ~15–30 min questions + research) → **plan-milestone-all** (stay — review and approve plans) → **autopilot** (configure and walk away; ralph runs in another terminal)
- **Existing:** Resume work → then runs the next logical command (discuss/plan/execute/verify/complete) and keeps going until you stop or the milestone is done

You never have to remember which command comes next. Say "stop" or "pause" anytime to end the flow.
