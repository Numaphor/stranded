...
OUTPUT_SIZE = (32, 32)
OPAQUE_MASK_SUM = OUTPUT_SIZE[0] - 1  # x + y >= OUTPUT_SIZE[0] - 1

PROJECT_ROOT = Path(__file__).resolve().parents[1]
SOURCES = (
    ("escaping_criticism", PROJECT_ROOT / "butano/examples/sp_direct_bitmap_bgs/graphics/escaping_criticism.bmp"),
    ("mr_and_mrs_andrews", PROJECT_ROOT / "butano/examples/sp_direct_bitmap_bgs/graphics/mr_and_mrs_andrews.bmp"),
)
...
def _flatten_palette_256(colors16: list[tuple[int, int, int]]) -> list[int]:
    palette = list(colors16)

    if len(palette) > 16:
        raise ValueError(f"Expected <= 16 palette entries, got {len(palette)}")
...
def _build_base_index_image(source: Path) -> tuple[Image.Image, list[tuple[int, int, int]]]:
...
def _validate_outputs(top: Image.Image, bottom: Image.Image) -> None:
...
def _write_sprite_json(path: Path) -> None:
    payload = {"type": "sprite", "width": OUTPUT_SIZE[0], "height": OUTPUT_SIZE[1]}
    path.write_text(json.dumps(payload, indent=4) + "\n", encoding="utf-8")


def _build_paths(name: str, source_path: Path) -> PaintingSprites:
    return PaintingSprites(
        name=name,
        source_path=source_path,
        top_bmp=OUTPUT_DIR / f"{name}_wall_top.bmp",
        top_json=OUTPUT_DIR / f"{name}_wall_top.json",
        bottom_bmp=OUTPUT_DIR / f"{name}_wall_bottom.bmp",
        bottom_json=OUTPUT_DIR / f"{name}_wall_bottom.json",
    )
...
def main() -> int:
    try:
        for name, source_path in SOURCES:
            _generate_one(_build_paths(name, source_path))
