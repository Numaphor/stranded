...
To update C++ headers too, run tools/update_room_models.py (it uses a
shared palette and correct scales; table/chair use --flip-z so they render
right-side up in the engine).
...
obj_to_header.py and the engine.

Usage:
    python split_room_obj.py [--input-obj PATH] [--input-mtl PATH] [--output-dir DIR]

Example:
    # From repo root, default: obj/room.obj, obj/room.mtl -> obj/*.obj
    python tools/split_room_obj.py

    # Custom paths
    python tools/split_room_obj.py --input-obj obj/room.obj --output-dir obj
"""
...
def write_object_obj(path, object_name, vertices, normals, faces, mtl_name, center_at_origin=False, flip_y=False):
    ...
    if center_at_origin and v_list:
        ...
        tx, ty, tz = -cx, -cy, -cz
        flip_y = False  # table/chair: flip so they render right-side up
...
def main():
    ...
    for obj_name, faces in objects:
        ...
        # RoomShell: no flip so OBJ is right-side up (same as table/chair). Table/Chair: center only.
        flip_y_room = False
