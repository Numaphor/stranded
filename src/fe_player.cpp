...
namespace fe
{
    ...
    void Player::update()
    {
        ...
        if (_hp <= 0 && _death_timer > 0)
        {
            ...
            if (_death_timer <= 0)
            {
                // Automatically trigger respawn after 3 seconds
                _reset_required = true;
            }
            // During death animation, prevent all input and movement
            _movement.stop_movement();

            // Ensure death animation is applied
            if (_movement.current_state() == PlayerMovement::State::DEAD)
            {
                update_animation();
            }

            // Make sure player is visible during death animation (override invulnerability blinking)
            set_visible(true);
            ...
            {
                _companion->update(pos(), true); // Tell companion player is dead
                _companion->set_visible(true);   // Keep companion visible during player death
            }
            return; // Exit early, don't process normal game logic during death
        }
