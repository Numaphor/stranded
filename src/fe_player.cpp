...
namespace fe {
    ...
    Hitbox Player::get_melee_hitbox() const {
        ...
        switch (dir) {
        case PlayerMovement::Direction::UP: hitbox_y -= range;
            hitbox_x -= width / 2;
            break;
        case PlayerMovement::Direction::DOWN: hitbox_y += range;
            hitbox_x -= width / 2;
            break;
        case PlayerMovement::Direction::LEFT: hitbox_x -= range;
            hitbox_y -= height / 2;
            break;
        case PlayerMovement::Direction::RIGHT: hitbox_x += range;
            hitbox_y -= height / 2;
            break;
        default:
            break; }
...
namespace fe {
    ...
    void PlayerAnimation::apply_state(PlayerMovement::State state, PlayerMovement::Direction direction) {
        ...
        if (state == PlayerMovement::State::DEAD ||
            ...
            state == PlayerMovement::State::CHOPPING) { make_anim_range_once(anim.speed, start_frame, start_frame + frame_count - 1); }
        else { make_anim_range(anim.speed, start_frame, start_frame + frame_count - 1); }
        _last_state = state;
...
namespace fe {
    ...
    void Player::handle_input() {
        ...
        if (_movement.current_state() == PlayerMovement::State::ROLLING) {
            ...
            switch (roll_dir) {
            case PlayerMovement::Direction::RIGHT: should_cancel = bn::keypad::left_pressed();
                break;
            case PlayerMovement::Direction::LEFT: should_cancel = bn::keypad::right_pressed();
                break;
            case PlayerMovement::Direction::UP: should_cancel = bn::keypad::down_pressed();
                break;
            case PlayerMovement::Direction::DOWN: should_cancel = bn::keypad::up_pressed();
                break; }
            if (should_cancel) {
                _movement.stop_action();
                _state.set_invulnerable(false); }
            ...
            if (!bn::keypad::select_held()) {
                if (!_hud.is_buff_menu_open()) {
                    ...
                    if (bn::keypad::a_pressed() || bn::keypad::l_pressed()) {
                        ...
                        PlayerMovement::State buff_state = PlayerMovement::State::IDLE;
                        switch (selected) {
                        case 0: buff_state = PlayerMovement::State::HEAL_BUFF;
                            break;
                        case 1: buff_state = PlayerMovement::State::ENERGY_BUFF;
                            break;
                        case 2: buff_state = PlayerMovement::State::POWER_BUFF;
                            break;
                        default:
                            break; }
        ...
        if (!performing_action && !reviving_companion && !_hud.is_buff_menu_open()) {
            ...
            else if (!should_run && _movement.is_state(PlayerMovement::State::RUNNING)) { _movement.start_action(PlayerMovement::State::WALKING, 0); }
        }
    ...
    void Player::toggle_gun() {
        ...
        else if (!_gun_active) { _gun_sprite.reset(); }
    }
...
namespace fe {
    ...
    void PlayerMovement::move_direction(Direction dir) {
        switch (dir) {
        case Direction::RIGHT: _dx = bn::clamp(_dx + acc_const, -max_speed, max_speed);
            break;
        case Direction::LEFT: _dx = bn::clamp(_dx - acc_const, -max_speed, max_speed);
            break;
        case Direction::UP: _dy = bn::clamp(_dy - acc_const, -max_speed, max_speed);
            break;
        case Direction::DOWN: _dy = bn::clamp(_dy + acc_const, -max_speed, max_speed);
            break;
        default:
            break; }
    ...
    void PlayerMovement::stop_running() {
        if (_current_state == State::RUNNING) { _current_state = (bn::abs(_dx) > movement_threshold || bn::abs(_dy) > movement_threshold) ? State::WALKING : State::IDLE; }
    }
...
namespace fe {
    ...
    void PlayerVFX::update(bn::fixed_point player_pos, PlayerMovement::State state, PlayerMovement::Direction direction) {
        if (should_show_vfx(state)) {
            ...
            if (is_attack && (direction == PlayerMovement::Direction::UP ||
                              direction == PlayerMovement::Direction::DOWN)) { vfx_pos = bn::fixed_point(player_pos.x() + 8, player_pos.y() + PLAYER_SPRITE_Y_OFFSET); }
            else { vfx_pos = bn::fixed_point(player_pos.x(), player_pos.y() + PLAYER_SPRITE_Y_OFFSET); }
    ...
    void PlayerVFX::apply_vfx_state(PlayerMovement::State state, PlayerMovement::Direction direction) {
        ...
        switch (state) {
        case PlayerMovement::State::SLASHING: if (direction == PlayerMovement::Direction::UP)
                make_vfx_anim_range_once(4, 480, 486);
            else if (direction == PlayerMovement::Direction::DOWN)
                make_vfx_anim_range_once(4, 192, 198);
            else
                make_vfx_anim_range_once(4, 336, 339);
            break;
        case PlayerMovement::State::ATTACKING: if (direction == PlayerMovement::Direction::UP)
                make_vfx_anim_range_once(4, 480, 486);
            else if (direction == PlayerMovement::Direction::DOWN)
                make_vfx_anim_range_once(4, 192, 198);
            else
                make_vfx_anim_range_once(4, 360, 364);
            break;
        case PlayerMovement::State::CHOPPING: if (direction == PlayerMovement::Direction::UP)
                make_vfx_anim_range_once(5, 456, 459);
            else if (direction == PlayerMovement::Direction::DOWN)
                make_vfx_anim_range_once(5, 168, 171);
            else
                make_vfx_anim_range_once(5, 336, 339);
            break;
        case PlayerMovement::State::HEAL_BUFF: make_vfx_anim_range(4, 24, 47);
            break;
        case PlayerMovement::State::DEFENCE_BUFF: make_vfx_anim_range(4, 48, 71);
            break;
        case PlayerMovement::State::POWER_BUFF: make_vfx_anim_range(4, 72, 95);
            break;
        case PlayerMovement::State::ENERGY_BUFF: make_vfx_anim_range(4, 96, 119);
            break;
        default:
            hide_vfx();
            break; }
    ...
    bool PlayerVFX::should_change_vfx(PlayerMovement::State state, PlayerMovement::Direction direction) const { return state != _last_vfx_state || direction != _last_vfx_direction; }
...
namespace fe {
    ...
    void PlayerCompanion::update(bn::fixed_point player_pos, bool player_is_dead) {
        ...
        if (_is_reviving) {
            ...
            else if (_text_sprites.size() > 0) { hide_revival_text(); }
        }
    ...
    void PlayerCompanion::update_position(bn::fixed_point player_pos) {
        ...
        if (player_distance > 8) {
            ...
            else { new_side = (offset.x() > 0) ? Position::RIGHT : Position::LEFT; }
    ...
    void PlayerCompanion::reset_text_positions() {
        if (_text_sprites.empty() || _text_original_offsets.empty())
            return;
        bn::fixed_point text_center = _death_position + bn::fixed_point(0, -20);
        for (int i = 0; i < _text_sprites.size() && i < _text_original_offsets.size(); ++i) { _text_sprites[i].set_position(text_center + _text_original_offsets[i]); }
    }
}