...
namespace fe {
    ...
    [[nodiscard]] int _get_map_cell(bn::fixed x, bn::fixed y, bn::regular_bg_ptr & , bn::span<const bn::regular_bg_map_cell> cells) {
    ...
    [[nodiscard]] bool _check_collisions_map(bn::fixed_point pos, Hitbox hitbox, directions direction, bn::regular_bg_ptr &map, Level level, bn::span<const bn::regular_bg_map_cell> cells) {
        ...
        for (int i = 0; i < 4; ++i) {
            ...
            if (_contains_cell(tile, level.floor_tiles())) { return true; }
        }
        return false; }
    Enemy::Enemy(int x, int y, bn::camera_ptr camera, bn::regular_bg_ptr map, ENEMY_TYPE type, int hp) : Entity(bn::fixed_point(x, y)),
        ...
        switch (_type) {
        case ENEMY_TYPE::SPEARGUARD: builder = bn::sprite_builder(bn::sprite_items::spearguard);
            break;
        case ENEMY_TYPE::SLIME: builder = bn::sprite_builder(bn::sprite_items::spearguard);
            break;
        case ENEMY_TYPE::MUTANT: builder = bn::sprite_builder(bn::sprite_items::spearguard);
            break;
        default:
    ...
    void Enemy::update(bn::fixed_point player_pos, const Level &level, bool player_listening) {
        ...
        else if (_dead && _sprite.has_value()) { _sprite->set_visible(true); }
    ...
    void Enemy::_update_spearguard_animation() {
        ...
        if (_dead) { desired_animation = AnimationState::DEAD; }
            ...
            _attack_timer--; }
                 ...
                 _state_machine.get_current_state_id() == EnemyStateId::RETURN_TO_POST) { desired_animation = AnimationState::RUN; }
        else { desired_animation = AnimationState::IDLE; }
        if (desired_animation != _current_animation) {
            _current_animation = desired_animation;
            switch (_current_animation) {
            case AnimationState::IDLE: _action = bn::create_sprite_animate_action_forever(
                    *_sprite,
                    12,
                    bn::sprite_items::spearguard.tiles_item(),
                    0, 1, 2, 3, 4, 5
                ...
                break;
            case AnimationState::RUN: _action = bn::create_sprite_animate_action_forever(
                    *_sprite,
                    8,
                    bn::sprite_items::spearguard.tiles_item(),
                    6, 7, 8, 9
                ...
                break;
            case AnimationState::ATTACK: _action = bn::create_sprite_animate_action_forever(
                    *_sprite,
                    6,
                    bn::sprite_items::spearguard.tiles_item(),
                    10, 11, 12, 13, 14
                ...
                break;
            case AnimationState::DEAD: _action = bn::create_sprite_animate_action_once(
                    *_sprite,
                    8,
                    bn::sprite_items::spearguard.tiles_item(),
