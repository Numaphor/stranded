...
namespace fe {
    ...
    fe::Scene fe::World::execute(bn::fixed_point spawn_location, int world_id) {
        ...
        while (true) {
            ...
            else { _continuous_fire_frames = 0; }
            ...
            switch (facing_dir) {
            case PlayerMovement::Direction::RIGHT: desired_lookahead = bn::fixed_point(CAMERA_LOOKAHEAD_X, 0);
                break;
            case PlayerMovement::Direction::LEFT: desired_lookahead = bn::fixed_point(-CAMERA_LOOKAHEAD_X, 0);
                break;
            case PlayerMovement::Direction::UP: desired_lookahead = bn::fixed_point(0, -CAMERA_LOOKAHEAD_Y);
                break;
            case PlayerMovement::Direction::DOWN: desired_lookahead = bn::fixed_point(0, CAMERA_LOOKAHEAD_Y);
                break;
            default:
                break; }
    ...
    void World::_init_world_specific_content(int world_id, bn::camera_ptr &camera, bn::regular_bg_ptr &bg, bn::sprite_text_generator &text_generator) {
        ...
        if (_merchant) {
            ...
            _merchant = nullptr; }
        switch (world_id) {
        case 0: _merchant = new MerchantNPC(bn::fixed_point(100, -50), camera, text_generator);
            _enemies.push_back(Enemy(0, -100, camera, bg, ENEMY_TYPE::SPEARGUARD, 3));
            _enemies.push_back(Enemy(50, -80, camera, bg, ENEMY_TYPE::SPEARGUARD, 3));
            _enemies.push_back(Enemy(-50, -120, camera, bg, ENEMY_TYPE::SPEARGUARD, 3));
            break;
        case 1: _enemies.push_back(Enemy(-100, -50, camera, bg, ENEMY_TYPE::SPEARGUARD, 2));
            _enemies.push_back(Enemy(80, -100, camera, bg, ENEMY_TYPE::SPEARGUARD, 2));
            break;
        case 2: _merchant = new MerchantNPC(bn::fixed_point(-80, 100), camera, text_generator);
